...
路径比较按不可变提交图实现。主线使用灰色轨道，个人分支使用独立颜色；每次成功结算生成一个不可修改节点，历史节点只能读取或作为新分支起点。第 1–32 周使用同一日历周主线快照作比较，分支进入第 33 周后固定比较主线第 32 周最终快照并标注主线已完成。节点详情包含行动链、决策依据、状态差异和文件差异；最多同时显示 3 条个人分支。MVP 不实现 merge/cherry-pick，从节点重试等价于创建新分支。
...
5. 组合语义：服务端按单条行动链内的文件、工具与技术和干系人组合推断私有管理动作及连接；跨行动链卡片不自动拼接成一个动作。
...
| `lab_round_drafts` | 云端自动保存未提交行动链与决策依据 | `branch_id`, `round_number`, `scenario_id`, `selected_card_ids_json`（扁平卡片兼容字段）, `connections_json`（结构化行动链兼容字段）, `reasoning_json` |
...
4. 每个情景的候选卡数量为 18–24，包含 5–7 个必要管理动作，最小正确行动链由 8–14 张卡构成。
...
**服务端结算兼容**：案例源文件继续保留私有执行动作卡和最小正确连接，用于确定性规则结算，但这些卡不投影到客户端。结算端只在同一条用户行动链中找齐某管理动作的可见支持卡后，推断对应私有执行动作和相邻连接。行动目标文本只用于表达和 AI 复盘，不参与关键词判分。
...
**已确认情景一必要动作**：澄清家庭成员共享需求及安全边界；对照批准需求与范围确认基线外变更；评估范围、进度、成本、安全、质量和干系人影响；形成变更请求并提交 CCB；纳入 1.1 并保护 1.0 试点基线；更新关联记录并向试点车主说明替代流程和明确承诺。当前情景规则源文件为 `content/lab-cases/car-control/v3/scenario-plan.json`。

**已确认情景一最小正确卡**：D16/D21 两张证据卡；T002 访谈、T010 备选方案分析、T053 多标准决策分析三张工具卡；创建变更请求、完成综合影响评估、纳入 1.1 并维持 1.0、更新记录并沟通替代流程四张私有执行卡；试点车主代表、产品负责人/业务分析师、技术负责人、DevOps/安全工程师、项目发起人五张干系人卡，共 14 张。用户本人是项目经理，不另设项目经理卡。D06/D14/D26 必须查看或通过仪表盘读取，但不要求拖入行动链。

**已确认情景一必要连接**：D21 连接访谈，D16/D21 连接备选方案分析与多标准决策分析；访谈和备选方案分析连接影响评估，备选方案分析连接变更请求，多标准决策分析连接纳入 1.1 决策，访谈连接记录与沟通；影响评估连接产品/技术/安全，变更请求连接产品/技术/安全/发起人，纳入 1.1 连接产品/发起人，记录与沟通连接试点车主代表/产品负责人。系统在服务端校验语义连接，不要求用户手动画线。

**已确认情景一候选池**：共 22 张卡。D06/D14/D26、T008 头脑风暴和市场发布负责人属于有帮助但非必要的可选项，前三项不扣正确性，后两项只增加少量管理负担；T098 进度压缩、立即开发家庭共享功能、未经分析直接拒绝反馈属于有害干扰项，分别触发过早进度压缩、未受控范围执行和干系人预期失管后果。
...
**已确认情景一有害项后果**：过早赶工增加 4 个加班人日和 9300 元 AC，短期预测压缩 2 个工作日但不推进风险或范围闭环；立即开发产生 12 个未批准范围人日和 18250 元 AC，挤占关键工作并预测延期 1 周，构成范围控制违规；直接拒绝反馈使试点代表从支持降为中立，增加 1 个逾期沟通项并使有效试点参与预测减少 30 人。三类后果可叠加，短期进度改善不能抵消治理缺口。

**已确认情景一终局**：W10 无有害项闭环为近主线成功；W11–W12 完成纠正且未继续未批准开发为绕路成功，仍可 W32 完成；发生未批准开发后完成纠正为延期成功，预测 W33 完成；到 W13 仍无 CCB 决定、累计未批准工作达到 36 人日，或连续 3 回合继续开发而不处理范围与安全治理时，分支立即以“范围治理失控”失败并进入复盘。
...
**确认决策**：MVP 的项目文件只读。用户可以打开、筛选、引用、选择和比较文件版本，通过行动链触发文件创建或更新，并查看系统生成的结构化差异；用户不能直接编辑文件字段。每回合三段式决策依据仍由用户必填。手工填写变更请求、风险应对计划等能力列入后续高级模式。
...
**已确认情景二必要管理动作**：第 17 周同时触发供应商接口延期与核心车端接口工程师被临时抽调。最小正确集合必须覆盖七项必要管理动作：核实两项事实并登记关联问题；重算关键路径、接口依赖、资源日历及里程碑影响；与供应商联合排障并确定最小可交付接口和分阶段日期；启用 Mock 接口保持独立开发与测试并行；安排替补工程师和结构化交接且不压缩集成与质量门禁；更新进度预测、风险登记册、责任分配和供应商绩效记录；向项目发起人、产品负责人及受影响团队沟通“顺延一周、质量优先”方案并取得一致。七项动作分别使用 `S2-A1` 至 `S2-A7`，统一维护在 `content/lab-cases/car-control/v3/scenario-plan.json`。

**已确认情景二最小正确卡片**：最小行动链包含 14 张卡。文件卡为 D08 问题日志、D14 项目进度计划和 D26 风险登记册；工具与技术卡为 T087 关键路径法、T114 问题解决；执行卡为约定最小接口与分阶段交付、启用 Mock 并行开发测试、安排替补与结构化交接、更新记录并沟通顺延一周方案；干系人卡为车端接口供应商项目经理、技术负责人、测试工程师、产品负责人/业务分析师和项目发起人。D17 项目团队派工单、D24 资源日历和 D29 进度预测是必须查看的场景线索，但不要求拖入最小行动链。

**已确认情景二最小正确连接**：最小闭环包含 21 条有向连接。文件到工具共 4 条：D08→问题解决、D14→关键路径法、D26→关键路径法、D26→问题解决。工具到行动共 6 条：关键路径法分别连接分阶段交付、Mock、替补交接和更新沟通，问题解决连接分阶段交付和更新沟通。行动到干系人共 11 条：分阶段交付连接供应商项目经理和技术负责人；Mock 及替补交接均连接技术负责人和测试工程师；更新沟通连接供应商项目经理、技术负责人、测试工程师、产品负责人和项目发起人。卡片选择正确但缺少必要连接时，只反馈相应闭环缺口，不判定为完整解决。

**已确认情景二候选卡池**：候选池共 22 张，即 14 张最小正确卡加 8 张干扰卡。有效但非必要卡为 D17 项目团队派工单、D24 资源日历、D29 进度预测、专家判断和车端接口集成工程师；使用后不判错，但分别增加 1 单位管理负载。三张有害卡为要求被抽调工程师加班兼顾两个项目、跳过部分集成测试以维持原上线日期、等待供应商完整接口交付后再恢复工作，分别触发人员过载、质量门禁绕过和被动串行等待后果。
...
**已确认情景二有害卡后果**：要求被抽调工程师加班兼顾两个项目会增加 6 个加班人日和 14,850 元成本，只形成 1 个工作日的表面压缩且不解决交接缺口，使 R03 残余风险升至 P4/I4，并增加疲劳与缺陷风险。跳过部分集成测试会让表面预测恢复至第 32 周，但触发质量红线，预测逃逸 1 个严重缺陷和 3 个主要缺陷；后续必须补测 10 个工作日并增加 32,000 元，当回合不得判定成功。等待完整接口后再恢复工作会产生 30 个受阻人日，使预测完工推迟至第 35 周、第 18 周 SPI 降至 0.89，并使 R02 转为失控。三类效果可叠加。

**已确认情景二终局规则**：第 18 周完成全部必要动作且未选择有害卡时归类为近主线成功，预测第 33 周完成；第 19–20 周补齐必要动作且没有遗留质量红线时归类为绕路成功，预测第 34 周完成；发生加班、被动等待或跳过测试后，只有完成全部纠偏、补测并将 R02/R03 恢复受控，才归类为延期成功，预测第 35–36 周完成。到第 21 周仍无可执行的供应商最小交付承诺且未启用 Mock、绕过质量门禁后继续推进未经验证构建、R02 或 R03 连续三个回合失控、预测完工晚于第 38 周、预测最终成本达到 286 万元中的任一条件成立，即判定情景失败，原因归类为“关键供应与资源约束失控”。

**已确认情景三必要管理动作**：第 25 周安全测试发现远程控制高危漏洞。最小正确集合必须覆盖七项必要管理动作：复核测试报告并复现漏洞，确认严重级别、影响范围和触发条件；登记安全问题并触发 R04/R10，立即冻结远程控制发布；综合评估范围、进度、成本、质量、安全及干系人影响；组织安全分诊与专家评审，制定修复、复测和残余风险处置方案；提交正式变更并完成 CCB 及上线阶段门决策，批准分阶段交付；通过功能开关和权限隔离确保只读/查看状态能力可独立安全上线；更新范围、需求追踪、风险、质量和发布资料，并向项目发起人、试点车主、运营及市场沟通预期。最优决策固定为“按期上线只读/查看状态能力，暂时关闭远程控制”。七项动作使用 `S3-A1` 至 `S3-A7`，统一维护在 `content/lab-cases/car-control/v3/scenario-plan.json`。

**已确认情景三最小正确卡片**：最小行动链包含 14 张卡。文件卡为 D32 测试与评估文件、D20 质量报告和 D26 风险登记册；工具与技术卡为 T133 测试/产品评估、T010 备选方案分析、T053 多标准决策分析；执行卡为冻结远程控制并创建正式变更、启用功能开关与权限隔离并验证只读能力、更新资料并沟通分阶段发布及修复计划；干系人卡为 DevOps/安全工程师、第三方安全测试负责人、技术负责人、产品负责人/业务分析师和项目发起人。D05 变更日志、D16 项目范围说明书、D22 需求跟踪矩阵和 D29 进度预测是必须查看的场景线索，但不要求拖入最小行动链。

**已确认情景三最小正确连接**：最小闭环包含 22 条有向连接。文件到工具共 5 条：D32/D20 连接测试/产品评估，D20/D26 连接备选方案分析，D26 连接多标准决策分析。工具到行动共 8 条：测试/产品评估连接冻结远程控制和隔离验证只读；备选方案分析连接三张执行卡；多标准决策分析同样连接三张执行卡。行动到干系人共 9 条：冻结远程控制连接安全工程师、技术负责人、产品负责人和项目发起人；隔离验证只读连接安全工程师、第三方安全测试负责人和技术负责人；更新沟通连接产品负责人和项目发起人。卡片正确但连接缺失时仅反馈对应闭环缺口。

**已确认情景三候选卡池**：候选池使用上限 24 张，即 14 张最小正确卡加 10 张干扰卡。有效但非必要卡为 D05 变更日志、D16 项目范围说明书、D22 需求跟踪矩阵、D29 进度预测、试点车主代表、运营与客服负责人、市场与发布负责人；使用后不判错，但分别增加 1 单位管理负载。有害卡为接受高危残余风险并照常上线远程控制、未经根因分析和完整复测直接热修复上线、暂停整个版本等待远程控制修复后统一上线，分别触发不安全发布、未经验证修复和无必要整体延期后果。
...
**已确认情景三有害卡后果**：接受高危残余风险并照常上线远程控制会使 R04 升至 P5/I5，第 28 周上线门直接退回且当回合不得成功；若未经验证构建进入试点则立即失败。未经根因分析和完整复测直接热修复会增加 12 人日和 26,400 元，复测预测重新打开 2 个主要安全缺陷，预测完工推迟至第 33 周，R04 保持已触发，且该动作不计为有效风险处置。暂停整个版本等待远程控制修复不会恶化安全风险，但会无必要阻塞安全的只读能力，增加 24 个协调/等待人日和 38,000 元，使整体上线推迟至第 35 周，并使试点参与人数预测下降 20%。三类效果可叠加。

**已确认情景三终局规则**：第 26 周完成全部必要动作、关闭远程控制、通过只读安全门且未选择有害卡时归类为近主线成功，第 32 周上线；第 27–28 周补齐动作并解除安全红线时归类为绕路成功，预测第 32–33 周完成；第 29–30 周完成全部纠偏、关闭远程控制并通过只读独立回归时归类为延期成功，预测第 33–35 周完成。未经验证的远程控制构建进入试点、第 28 周阶段门仍提交启用远程控制且 R04 为极高、安全红线连续两个回合未解除、到第 30 周仍无批准的安全发布范围、预测完工晚于第 38 周、预测最终成本达到 286 万元中的任一条件成立，即判定情景失败，原因归类为“安全治理与发布控制失控”。

**已确认情景事件发现规则**：用户点击“从这里接手”后不弹出事件答案，界面只通过收件箱红点、干系人消息、报告更新或仪表盘异常提示变化，用户必须主动打开材料发现问题。每个情景固定包含 3 条主要线索、1 条交叉验证线索和 1 个仪表盘异常；材料只陈述事实，不直接标注风险类别、正确工具或推荐决策。系统记录用户查看材料的时间、顺序和完整性，作为情景结束时 AI 评价“观察是否充分”的证据，但即时反馈仍只显示客观后果和缺口类别。
...
**已确认决策依据填写规则**：每回合分别填写“我观察到的关键信号”“我判断的主要风险/根因”“我选择这些行动的原因”三个字段，三项全部必填且每项 20–500 字。用户可引用已查看事件材料、项目文件和已选择行动卡，前端显示引用标签；草稿自动保存，任何字段不合规时禁止提交并就地提示。规则引擎不按关键词判分，也不要求标准术语；AI 仅评价判断与已查看证据、行动链及客观结果的一致性，避免奖励单纯的语言修饰。
...
**已确认 Git 式路径对比规则**：主线为灰色轨道，个人分支分别着色，每次提交形成不可修改节点且接手点为分叉点；节点展示行动链、决策依据、状态差异和文件差异。W1–W32 与主线同周快照比较，W33 后固定与主线 W32 最终快照比较并标注主线已完成。比较维度为进度、成本、范围、质量、安全、风险、资源、干系人和治理文件；最多并列 3 条个人分支，不支持合并或挑选节点，从历史节点重试时创建新分支。
...
1. 编制完整干系人名录和 RACI 基线，作为参与度与职责图表的数据源。
2. 继续完善 `car-control:v1` 案例源文件，先覆盖第 9 周情景一及相关文件。
